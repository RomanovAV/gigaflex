...
Принятые решения `fixed` и `rejected` сохраняются в компактной памяти текущего
запуска и передаются следующим review- и synthesis-проходам. Это не позволяет
агентам переоткрывать решённые замечания только ради другой допустимой
конвенции; актуальное состояние репозитория при этом остаётся источником истины.
Первый review проверяет весь накопленный diff. После synthesis следующий проход
проверяет только diff этого synthesis и файлы из его `fixed`/`confirmed` ledger;
он не начинает новый полный аудит исходного изменения.
По умолчанию доступно 10 repair-циклов. Лимит можно изменить параметром
`--review-iterations N` или настройкой `review_iterations`. После последнего
разрешённого repair GigaFlex всегда выполняет дополнительный read-only
verification-проход: успешные исправления поэтому не считаются ошибкой только
из-за исчерпания бюджета, а оставшиеся findings не запускают скрытый 11-й repair.

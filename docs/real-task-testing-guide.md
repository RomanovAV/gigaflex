...
свободный итоговый текст запускают узкий автоматический reconciliation-проход
по тем же findings и файлам. Если повторный ledger тоже невалиден, runner
переходит к следующей обычной review-итерации в пределах `review_iterations`,
а не останавливается для ручного вмешательства. Источником истины служит набор
решений, а не сигнал `<<<GIGALPHEX:REVIEW_DONE>>>`: пустой или полностью
`rejected` ledger завершает review, `fixed` и `confirmed` запускают следующий
независимый проход, а `blocked` останавливает прогон с указанной причиной.
